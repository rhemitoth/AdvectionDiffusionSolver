...
for j in range(0,Nt-1):
    for i in range(0,Nx-1):
        ...
        if i == 0:
            ...
            advection = -dt*((1/(2*dx))*(c_xplus1 * u[j][1][i + 1] - c_xminus1 * u[j][1][-1]))
            ...
            diffusion = r * (u[j][1][0] - 2 * u[j][1][i] + u[j][1][i - 1])
            advection = -dt*((1/(2*dx))*(c_xplus1 * u[j][1][0] - c_xminus1 * u[j][1][i-1]))
            ...
            diffusion = r * (u[j][1][i + 1] - 2 * u[j][1][i] + u[j][1][i - 1])
            advection = -dt*((1/(2*dx))*(c_xplus1 * u[j][1][i+1] - c_xminus1 * u[j][1][i-1]))
